...
class VocabItem:
    def __init__(self, word):
        ...
        # from the root to the word (leaf)
...
class Vocab:
    def __init__(self, fi, min_count):
        ...
        # assert self.word_count == sum([t.count for t in self.vocab_items]),
        # 'word_count and sum of t.count do not agree'
        logging.info('Total words in training file: %d', self.word_count)
        logging.info('Total bytes in training file: %d', self.bytes)
    ...
    def __sort(self, min_count):
        ...
        logging.info('Unknown vocab size: %d', count_unk)
...
class UnigramTable:
    ...
    def __init__(self, vocab):
        ...
        table_size = np.int(1e8)  # Length of the unigram table
        table = np.zeros(table_size, dtype=np.uint32)

        logging.info('Filling unigram table')
...
def init_net(dim, vocab_size):
    ...
    tmp = random.uniform(low=-0.5/dim, high=0.5/dim, size=(vocab_size, dim))
...
def save(vocab, syn0, fo, binary):
    logging.info('Saving model to: %s', fo)
    dim = len(syn0[0])
    if binary:
        fo = open(fo, 'wb')
        fo.write('%d %d\n' % (len(syn0), dim))
...
def train(fi, fo, cbow, neg, dim, alpha, win, min_count, num_processes, binary):
    ...
    table = None
    if neg > 0:
        logging.info('Initializing unigram table')
        table = UnigramTable(vocab)
    else:
        logging.info('Initializing Huffman tree')
    ...
    t1 = time.time()
    logging.info('')
    logging.info('Completed training. Training took %s %s', (t1 - t0) / 60, 'minutes')

    # Save model to file
    save(vocab, syn0, fo, binary)


def test(f, dim, topn=20):
    f = open(f, 'r')
    ...
    for line in f:
        if len(line.split()) != 1 + dim:
            continue
        token = line.split()[0]
        vector = [float(ftr) for ftr in line.split()[1:]]
        vector = np.array(vector)
        vocab[token] = vector

    # vec_quest = vocab['kings'] - vocab['queens'] + vocab['females']
    # vec_quest = vocab['everyone'] - vocab['everybody'] + vocab['yes']
    vec_quest = vocab['everyone'] - vocab['everybody'] + vocab['Monday']

    tokens = vocab.keys()
    dist = [0]*len(tokens)
    for i in range(len(tokens)):
        vec_token = vocab[tokens[i]]
        dist[i] = np.linalg.norm(vec_quest-vec_token)

    idx_dist = np.argsort(dist)

    logging.info('Top %s tokens:', topn)
    for j in range(topn):
        idx = idx_dist[j]
        token = tokens[idx]
        distj = dist[idx]
        logging.info('token: %s, dist: %s', token, distj)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-train', help='Training file', dest='fi', default='./enwik8')
    parser.add_argument('-model', help='Output model file', dest='fo', default='./word2vec2.model')
    parser.add_argument('-cbow', help='1 for CBOW, 0 for skip-gram', dest='cbow', default=0, type=int)
    parser.add_argument('-negative', help='Number of negative examples (>0) for negative sampling, 0 for hierarchical softmax',
                        dest='neg', default=0, type=int)
    ...
    parser.add_argument('-processes', help='Number of processes', dest='num_processes', default=multiprocessing.cpu_count(), type=int)
